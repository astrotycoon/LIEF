...
class TestAddSegment(TestCase):
    ...
    def test_simple(self):
        ...
        for i in range(4):
            ...
            segment                 = target.add_segment(segment)
    ...
    #        segment = libc.add_segment(segment)

    #        new_ep = (stub.header.entrypoint - original_va) + segment.virtual_address

    #        if libc.has_dynamic_entry(lief.ELF.DYNAMIC_TAGS.INIT_ARRAY):
    #            init_array = libc.dynamic_entry_from_tag(lief.ELF.DYNAMIC_TAGS.INIT_ARRAY)
    #            callbacks = init_array.array
    #            callbacks[0] = new_ep
    #            init_array.array = callbacks

    #        if libc.has_dynamic_entry(lief.ELF.DYNAMIC_TAGS.INIT):
    #            init = libc.dynamic_entry_from_tag(lief.ELF.DYNAMIC_TAGS.INIT)
    #            init.value = new_ep
